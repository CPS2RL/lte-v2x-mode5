...
def get_total_num_vehicle_with_succ_rec(all_data_list, tx_sf, tx_id, dist_min, dist_max):
    num_vehicle_succ = 0
    total_num_vehicle = 0
...
def calculate_avg_prr_by_dist(all_data_list, dist_min, dist_max, optional_txt=''):
    ...
    for txsfl in tx_sf_list_cond:
        ...
        total_num_vehicle, num_vehicle_succ = get_total_num_vehicle_with_succ_rec(
                                        all_data_list, tx_sf, tx_id, dist_min, dist_max)
    ...
    print "<=== PRR:", optional_txt, "-> Finished distance range: [", dist_min, ",", dist_max, ") ===>"
...
def calculate_and_write_prr(input_trace_filename, output_filename, optional_txt=''):
    amin = 0
    avgprr_amax = 500
    distgap = 20
    ...
    # get the traces and write to file
    data, _ = dp.parse_per_raw_data_with_tx_packet_sf(input_trace_filename)
    avg_prr = calculate_avg_prr_for_all_dist(data, avgprr_distminlist, distgap, optional_txt)
    write_prr_trace_to_file(output_filename=output_filename, dist_list=avgprr_distminlist,
                            dist_gap=distgap, avg_prr=avg_prr)


if __name__ == '__main__':

    # do for 10 MHz
    calculate_and_write_prr(input_trace_filename="../v2x_simulator/v2x_traces/kpi_traces/v2x_default_log_per.txt",
                            output_filename="../v2x_simulator/v2x_traces/kpi_traces/distVSprr_v2x_default_log_per.txt",
                            optional_txt='10MHz_HARQ_DIS_RC_DIS_PC_DIS')
